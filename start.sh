#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# gumes marketplace — lancement complet avec Docker (dev)
#
# Chaque démarrage réinitialise la base : teardown + up + migrations + seed
# + import des images. Puis lance le serveur (Express :3000) et le client
# (Vite :5173) en parallèle. Ctrl-C arrête les deux proprement.
#
# Pour un démarrage sans Docker (PostgreSQL déjà installé nativement), voir
# ./start_dev.sh (Linux/macOS) ou start_dev.bat (Windows).
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail
cd "$(dirname "$0")"
...
info "Démarrage de PostgreSQL + Adminer…"
docker compose up -d
...
# ── Migrations + seed + images ───────────────────────────────────────────────
info "Préparation de la base (migrations + seed + images)…"
npm run db:prep
...
bold "    ▸ Adminer  http://localhost:8080  (System: PostgreSQL, Server: db, Utilisateur: gumes, BDD: gumes_marketplace)"
bold ""
